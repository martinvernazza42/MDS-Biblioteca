...
class LibroCreateView(CreateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            form.add_error(
                "isbn",
                f"Ya existe un libro con este ISBN: {libro_existente.titulo} - {libro_existente.autor}",
            )
...
class PrestarLibroView(TemplateView):
    ...
    def post(self, request, pk):
        try:
            ...
            messages.success(
                request,
                f"Libro prestado a {socio.nombre_completo}. Devolución prevista: {prestamo.fecha_devolucion_prevista}",
            )
    ...
    def post(self, request, pk):
        try:
            ...
            prestamo.registrar_devolucion(
                buen_estado=buen_estado, importe_multa=importe
            )
            ...
            if buen_estado:
                ...
                messages.warning(
                    request, "Libro devuelto con daños. Se ha registrado una multa."
                )
...
class CambiarEstadoLibroView(View):
    def post(self, request, pk):
        try:
            ...
            messages.success(
                request, f"Estado del libro cambiado a {libro.get_estado_display()}"
            )
